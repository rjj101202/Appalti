...
export async function POST(request: NextRequest, { params }: { params: { id: string; stage: string } }) {
  try {
    ...
    const res = await db.collection('bids').updateOne(
      ...
      { $set: { 'stages.$.status': 'approved', 'stages.$.approvedAt': new Date(), 'stages.$.approvedBy': new ObjectId(auth.userId), updatedAt: new Date(), updatedBy: new ObjectId(auth.userId) } }
    );
