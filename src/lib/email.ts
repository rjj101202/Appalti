...
export function buildInviteEmailHtml(inviteUrl: string, context: { companyName: string; inviteeEmail: string; }) {
  ...
  <div style="font-family: Inter, system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif; line-height:1.5;">
    ...
    <p style="margin:24px 0;"><a href="${inviteUrl}" style="background:#7c3aed;color:#fff;padding:10px 16px;border-radius:8px;text-decoration:none;display:inline-block;">Uitnodiging accepteren</a></p>
...
export function buildOwnerApprovalNotificationHtml(details: { companyName: string; requesterEmail: string; teamUrl: string; }) {
  ...
  <div style="font-family: Inter, system-ui, -apple-system, Segoe UI, Roboto, Arial, sans-serif; line-height:1.5;">
    ...
    <p style="margin:24px 0;"><a href="${details.teamUrl}" style="background:#7c3aed;color:#fff;padding:10px 16px;border-radius:8px;text-decoration:none;display:inline-block;">Open teambeheer</a></p>
  </div>
  `;
}
